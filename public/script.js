document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleImageLoad(limit) {
        ...
        // listens for 2 events: * onload when it successfully loads images, * one for when image cannot load image; error
    ...
    async function searchPodcasts() {
        ...
        try {
            ...
            const titles = new Set(); // prevents duplicates of podcasts
    ...
    function deleteFromQueue(episode) {
        ...
        const queueElements = document.querySelectorAll('.queue-item'); // all of elements with className of .queue-item
    ...
    // Saving items to queue
    function saveQueue(episode) {
        queueItems.push(episode);
        localStorage.setItem('queue', JSON.stringify(queueItems)); // stringify to store strings not objects
    }

    // Load saved queue to repopulate the queue container
    function loadQueue() {
    ...
    function loadPodcast(episode) {
        ...
        // Reset player everytime it loads
    ...
    // Save player state to local storage every 5 seconds
    setInterval(() => {
        if (isPlaying) {
            const playerState = {
    ...
    function loadPlayerState() {
        ...
        if (savedState) {
            ...
            formatTime(savedState.currentTime, currentTimeEl); // this also updates the current time UI element
